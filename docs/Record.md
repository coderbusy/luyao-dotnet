...
#### 是否应在二进制序列化阶段启用压缩（决策参考）

结论建议：**默认不内置压缩能力**，按场景在外层流（如 `GZipStream` / `BrotliStream`）按需启用。

支持“启用压缩”的理由：

- **显著降低传输体积**：`Record/RecordSet` 常含重复列名、字符串和结构化数据，压缩收益通常较高，尤其适合网络传输与持久化归档。
- **降低带宽成本**：跨服务调用或远程存储场景中，可直接减少网络 I/O 和存储占用。
- **外层可配置策略**：通过外层流压缩可按场景选算法、级别、CPU/体积权衡，不破坏当前二进制格式。

支持“默认不启用压缩”的理由：

- **CPU 开销与延迟**：压缩/解压会增加 CPU 成本，小数据量时可能得不偿失，影响低延迟路径。
- **内存与本地调用收益有限**：进程内传递、短生命周期对象、少量数据场景通常不需要压缩。
- **调试与诊断复杂度上升**：原始字节不再直观，排查序列化问题时可读性下降。
- **格式演进复杂化**：若把压缩做成内置格式特性，需要额外头信息、版本兼容与回退逻辑。

推荐策略：

1. `Record/RecordSet` 的 `WriteTo/ReadFrom/ToBytes/FromBytes` 保持“纯二进制格式”职责，不绑定压缩算法。
2. 在网络/磁盘等高体积场景，由调用方在外层流启用压缩。
3. 当数据量较小或 CPU 更敏感时，保持未压缩路径。
